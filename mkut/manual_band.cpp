...
pair<block, int> paint_column(int x, int y, vector<int>& cut_x, vector<int>& cut_y, block prev_node, block node, int& id) {
    ...
    if (y + 2 < cut_y.size()) {
        cost += ycut_block(node, cut_y[y + 1]);
        node.y = cut_y[y + 1];
        node.id += ".0";
    ...
    auto r = paint_column(x, y + 1, cut_x, cut_y, node, next_node, id);
...
int paint(int x, vector<int>& cut_x, vector<int>& cut_y, block painted_node, block node, int& id) {
    ...
    // paint_column
    auto ret = paint_column(x, 0, cut_x, cut_y, block("", 0, 0), node, id);
    node = ret.first;
    int cost = ret.second;
