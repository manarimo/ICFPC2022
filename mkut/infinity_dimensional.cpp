...
pair<block, int> paint_column(int x, int y, vector<int>& cut_x, vector<int>& cut_y, bool asc_x, bool asc_y, bool flip, block node, int& id, int draw_from, color current_color, string& ans) {
    ...
    auto c = flip ? fast_get_color(b, l, t, r) : fast_get_color(l, b, r, t);
    if (y > 0 && !c.similar(current_color)) {
        int b2 = min(cut_y[draw_from], cut_y[y]), t2 = max(cut_y[draw_from], cut_y[y]);
        auto new_c = flip ? fast_get_color(b2, l, t2, r) : fast_get_color(l, b2, r, t2);
        double expected_cost = (draw_from > 0 ? round(CANVAS_SIZE / node.size() * 7) : 0)
            + round(CANVAS_SIZE / (asc_y ? ((flip ? node.x : node.y) - cut_y[draw_from]) : cut_y[draw_from]) / (flip ? node. y : node.x) * 5)
            + (draw_from > 0 ? round(CANVAS_SIZE / max(cut_y[draw_from], (flip ? node.x : node.y) - cut_y[draw_from]) / (flip ? node.y : node.x) * 1) : 0);

        double expected_gain = (flip ? similarity_cost(b2, l, t2, r) : similarity_cost(l, b2, r, t2))
            - (flip ? similarity_cost(b2, l, t2, r, new_c) : similarity_cost(l, b2, r, t2, new_c));

        if (expected_gain > expected_cost) {
            // cut bottom row
            block node0 = node, node1 = node;
            if (draw_from > 0) {
                cost += flip ? xcut_block(node, cut_y[draw_from], ans) : ycut_block(node, cut_y[draw_from], ans);

                node0.id += ".0"; node1.id += ".1";
                if (flip) {
                    node0.x = cut_y[draw_from];
                    node1.left = cut_y[draw_from];
                    node1.x -= node0.x;
                } else {
    ...
    if (y + 2 == cut_y.size()) {
        ...
        auto new_c = flip ? fast_get_color(b3, l, t3, r) : fast_get_color(l, b3, r, t3);
        double expected_cost = (draw_from > 0 ? round(CANVAS_SIZE / node.size() * 7) : 0)
            + round(CANVAS_SIZE / (asc_y ? ((flip ? node.x : node.y) - cut_y[draw_from]) : cut_y[draw_from]) / (flip ? node.y : node.x) * 5)
            + (draw_from > 0 ? round(CANVAS_SIZE / max(cut_y[draw_from], ((flip ? node.x : node.y) - cut_y[draw_from])) / (flip ? node.y : node.x) * 1) : 0);

        double expected_gain = (flip ? similarity_cost(b3, l, t3, r) : similarity_cost(l, b3, r, t3))
            - (flip ? similarity_cost(b3, l, t3, r, new_c) : similarity_cost(l, b3, r, t3, new_c));
    ...
    auto res = paint_column(x, y + 1, cut_x, cut_y, asc_x, asc_y, flip, node, id, draw_from, c, ans);

    return make_pair(res.first, res.second + cost);
}

int paint(int x, vector<int>& cut_x, vector<int>& cut_y, bool asc_x, bool asc_y, bool flip, block node, int& id, string& ans) {
    ...
    block node0 = node, node1 = node;
    if (x > 0) {
        cost += flip ? ycut_block(node, cut_x[x], ans) : xcut_block(node, cut_x[x], ans);
        node0.id += ".0"; node1.id += ".1";
        if (flip) {
            node0.y = cut_x[x]; node1.y -= cut_x[x];
            node1.bottom = cut_x[x];
        }
        else {
    ...
    auto ret = paint_column(x, 0, cut_x, cut_y, asc_x, asc_y, flip, next_node, id, 0, color(0,0,0,0), ans);
    node = ret.first;
    cost += ret.second;
    ...
    if (x > 0) {
        ...
        if (flip) {
            node.y = other_node.y + node.y;
            node.bottom = min(other_node.bottom, node.bottom);
        } else {
    ...
    cost += paint(x + 1, cut_x, cut_y, asc_x, asc_y, flip, node, id, ans);
...
int main() {
    ...
    for (int m = 0; m < 2; m++) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            string ans;
            int id = 0;
            int cost = paint(0, flip ? cut_y : cut_x, flip ? cut_x : cut_y, flip ? asc_y : asc_x, flip ? asc_x : asc_y, flip, block("0", MAX_W, MAX_H, 0, 0), id, ans);
