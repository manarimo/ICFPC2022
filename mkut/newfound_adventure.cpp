...
struct solver {
    ...
    int current[MAX_H][MAX_W][4];
    int id;

    ostream& out;
    int cost = 0;

    solver(const problem& P, const setting& S, ostream& out) : P(P), S(S), out(out), id(P.initial_id) {
        for (int k = 0; k < 4; k++) {
            for (int i = 0; i < P.height; i++) {
                for (int j = 0; j < P.width; j++) {
                    current[i][j][k] = P.initial[i][j][k];
                }
            }
        }
    }
    ...
    pair<block, block> xcut_block(const block& node, int x) {
        ...
        // build response
        block node0(node.id + ".0", node.left, node.bottom, x, node.height);
        block node1(node.id + ".1", x, node.bottom, node.width - x, node.height);
    ...
    pair<block, block> ycut_block(const block& node, int y) {
        ...
        // build response
        block node0(node.id + ".0", node.left, node.bottom, node.width, y);
        block node1(node.id + ".1", node.left, y, node.width, node.height - y);
    ...
    block merge_block(const block &node1, const block &node2) {
        ...
        // build response
        block ret(to_string(++id),
            min(node1.left, node2.left),
            min(node1.bottom, node2.bottom),
            node1.left == node2.left ? node1.width : node1.width + node2.width,
            node1.bottom == node2.bottom ? node1.height : node1.height + node2.height
        );
    ...
    void paint_block(const block& node, const color& c) {
        ...
        for (int x = node.left; x < node.left + node.width; x++) {
            for (int y = node.bottom; y < node.bottom + node.height; y++) {
                current[y][x][0] = c.r;
                current[y][x][1] = c.g;
                current[y][x][2] = c.b;
                current[y][x][3] = c.a;
    ...
    double similarity_cost(int x1, int y1, int x2, int y2) const {
        ...
        for (int x = x1; x < x2; x++) {
            for (int y = y1; y < y2; y++) {
                double d = 0;
                for (int k = 0; k < 4; k++) {
                    d += (P.target[y][x][k] - current[y][x][k]) * (P.target[y][x][k] - current[y][x][k]);
                }
                diff += sqrt(d) * 0.005;
            }
        }
