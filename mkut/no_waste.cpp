...
struct color {
    ...
    bool similar(const color& c) const {
        return (r - c.r) * (r - c.r) + (g - c.g) * (g - c.g) + (b - c.b) * (b - c.b) + (a - c.a) * (a - c.a) < 400;
    }
};
...
pair<block, int> paint_column(int x, int y, vector<int>& cut_x, vector<int>& cut_y, block node, int& id, int draw_from, color current_color) {
    ...
    if (y > 0 && !c.similar(current_color)) {
        ...
        double expected_gain = similarity_cost(cut_x[x], cut_y[draw_from], cut_x[x + 1], cut_y[y])
    ...
    if (y + 2 == cut_y.size()) {
        ...
        double expected_gain = similarity_cost(cut_x[x], cut_y[draw_from], cut_x[x + 1], cut_y[y + 1])
...
int main() {
    ...
    // #6
    vector<int> cut_y = {0, 20, 22, 29, 36, 44, 51, 58, 66, 72, 79, 85, 95, 100, 106, 119, 123, 157, 167, 182, 196, 211, 247, 250, 251, 262, 269, 283, 298, 305, 312, 327, 400};
    vector<int> cut_x = {0, 15, 51, 66, 87, 102, 122, 123, 124, 125, 153, 167, 182, 196, 211, 226, 240, 255, 272, 285, 304, 317, 342, 364, 379, 400};
    // reverse(cut_x.begin(), cut_x.end());
    // reverse(cut_y.begin(), cut_y.end());
    // for (int i = 0; i < cut_x.size(); i++) cut_x[i] = 400 - cut_x[i];
    // for (int i = 0; i < cut_y.size(); i++) cut_y[i] = 400 - cut_y[i];
